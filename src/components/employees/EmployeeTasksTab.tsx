...
import { CalendarIcon, Plus, Clock, User, AlertCircle, CheckCircle2, XCircle, Pause, Filter, MoreVertical, Eye, Edit, Trash } from 'lucide-react';
...
const EmployeeTasksTab: React.FC<EmployeeTasksTabProps> = ({ canManageEmployees, onResendInvitation }) => {
  ...
  const { user } = useAuth();
  const { canManageTimeTracking, isRegularEmployee, getEffectiveWarehouseRole, canUpdateTaskStatus } = useUserPermissions();
