        ...
        <Card className="bg-neutral-900/50 border-neutral-800">
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-neutral-300">Consolidated billing data across all warehouses</span>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="bg-neutral-900/50 border-neutral-800">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-neutral-400">Total Profit</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-neutral-900/50 border-neutral-800">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-neutral-400">Total Cost</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-neutral-900/50 border-neutral-800">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-neutral-400">Profitable Orders</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="bg-neutral-900/50 border-neutral-800">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-neutral-300">Billing Revenue</CardTitle>
            <DollarSign className="h-4 w-4 text-emerald-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-white">{formatCurrency(totalRevenue)}</div>
            <p className="text-xs text-neutral-400">From {invoices.length} invoices</p>
          </CardContent>
        </Card>

        <Card className="bg-neutral-900/50 border-neutral-800">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-neutral-300">Stock Value</CardTitle>
            <Package className="h-4 w-4 text-gray-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-white">{formatCurrency(totalStockValue)}</div>
            <p className="text-xs text-neutral-400">Current inventory value</p>
          </CardContent>
        </Card>

        <Card className="bg-neutral-900/50 border-neutral-800">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-neutral-300">Pending Invoices</CardTitle>
            <FileText className="h-4 w-4 text-amber-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-white">{pendingInvoices}</div>
            <p className="text-xs text-neutral-400">Awaiting payment</p>
          </CardContent>
        </Card>

        <Card className="bg-neutral-900/50 border-neutral-800">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-neutral-300">Collection Rate</CardTitle>
          ...
          <CardContent className="space-y-3">
            {invoices.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-neutral-400">
                <FileText className="h-12 w-12 mb-3 text-neutral-500" />
              ...
              invoices.slice(0, 5).map((invoice) => (
                <div key={invoice.id} className="flex items-center justify-between p-3 bg-neutral-800/30 rounded-lg">
                  <div>
                    <p className="text-sm font-medium text-white">{invoice.invoice_number}</p>
                    <p className="text-xs text-neutral-400">Due: {new Date(invoice.due_date).toLocaleDateString()}</p>
                    {isInCorporateOverview && invoice.warehouse_id && (
                      <p className="text-xs text-neutral-500">WH: {invoice.warehouse_id.slice(0, 8)}</p>
                    )}
                  </div>
                  <div className="flex items-center space-x-2">
                    <Badge className={
          ...
          <CardContent className="space-y-3">
            {billingRates.length === 0 ? (
              ...
              billingRates.slice(0, 5).map((rate) => (
                <div key={rate.id} className="flex items-center justify-between p-3 bg-neutral-800/30 rounded-lg">
                  <div>
                    <p className="text-sm font-medium text-white capitalize">{rate.service_type}</p>
                    <p className="text-xs text-neutral-400">{rate.rate_type.replace('_', ' ')}</p>
                    {isInCorporateOverview && rate.warehouse_id && (
                      <p className="text-xs text-neutral-500">WH: {rate.warehouse_id.slice(0, 8)}</p>
                    )}
                  </div>
                  <div className="text-right">
                    <span className="text-sm font-medium text-white">{formatCurrency(rate.rate_amount)}</span>
                    {rate.unit && <p className="text-xs text-neutral-400">per {rate.unit}</p>}
                  </div>
                </div>
              ))
            )}
