-- Drop and recreate the calculate_warehouse_metrics_enhanced function with real data
DROP FUNCTION IF EXISTS calculate_warehouse_metrics_enhanced(uuid);

CREATE OR REPLACE FUNCTION calculate_warehouse_metrics_enhanced(warehouse_uuid uuid DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    WHERE p.company_id = ANY(v_company_ids)
      ...
      AND p.quantity >= COALESCE(t.min_quantity, 0);
